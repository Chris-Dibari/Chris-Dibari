# ¡Hola! Soy Christian Dibari 👋

Estudiante de Ciencias del Mar apasionado sobre todo por la ingeniería costera, el análisis de datos oceanográficos y la hidráulica ambiental. 
Mi objetivo es especializarme en la gestión, modelado y diseño de obras e infraestructuras del litoral mediante el Máster de Costas y Puertos 
de la Universidad de Cantabria.

# Áreas de Interés 🌊

- 📈 Análisis de series temporales de oleaje, viento y mareas.
- 💻 Modelado numérico de propagación de ondas y morfodinámica de playas.
- 🗺️ Sistemas de Información Geográfica (SIG) aplicados a la gestión costera.
- ⛰️ Gestión de riesgos provocados por eventos extremos.

# 🛠️ Herramientas y Lenguajes (En aprendizaje activo)

- Lenguajes: Python (NumPy, Pandas, Matplotlib), R y RStudio, Matlab.
- Software/Herramientas: QGIS, GitHub Copilot.

## 🌊 Proyectos Destacados y Logros Académicos

### 🔬 Proyecto BRISA — Análisis Ambiental Litoral
*   **Resumen:** Coautor en la presentación científica internacional para la **Conferencia de Utrech (2026)**. 
*   **Material Oficial:** Incluye el registro formal de la contribución y el documento científico en formato PDF.
*   🔗 **[Explorar códigos y ver publicación detallada →](https://github.com)**

### 📢 Divulgación en el Instituto de Investigación Marina (INMAR)
*   **Resumen:** Colaborador activo en **La Noche Europea de los Investigadores** (Septiembre, 2026) junto al **Dr. Jesús Ruiz González** (Dpto. de Tecnologías del Medio Ambiente). 
*   **Material Oficial:** Resúmenes del taller, infografías didácticas y registros del evento.
*   🔗 **[Ver detalles del voluntariado y material didáctico →](https://github.com)**

### 📊 Base de Datos Académica — 4º Curso de Ciencias del Mar
*   **Resumen:** Repositorio centralizado de mi formación. Contiene la digitalización completa de mis asignaturas utilizando Markdown.
*   🔗 **[Acceder al repositorio completo de asignaturas →](https://github.com)**

# 📧 Contacto: 

chris.dibari@alum.uca.es